...
const createChat = TryCatch(async (req, res, next) => {
  ...
  const usersExistingChats = await db.query.chatMembers.findMany({
    // columns:{chatId:true},
    where: (chatMembers, { inArray, eq }) =>
      inArray(chatMembers.chatId, ExistingChatsIds) &&
  ...
  emitEvent(req, REFETECH_CHATS, [userId, myId], false);
...
const getChatDetails = TryCatch(async (req, res, next) => {
  ...
  const members = await db.query.chatMembers.findMany({
    where: (chatMembers, { eq, ne, and }) =>
      and(
        eq(chatMembers.chatId, chatId),
        ne(chatMembers.userId, res.locals.userId)
      ),
    with: {
      user: {
        columns: {
...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChat = await db.query.chatMembers.findMany({
    where: (chatMembers, { inArray, ne, and }) =>
    ...
    with: {
      ...
      chat: {
        columns: { id: true, lastMessage: true, lastSent: true },
      },
    },
    columns: {
      unreadCount: true,
  ...
  // const unreadChatCount = await db.query.chatMembers.findMany({
  //     where:(chatId)
  // })
